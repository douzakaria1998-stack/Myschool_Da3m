...
export default function AttendancePage() {
  ...
  const handleAutoGenerateDates = () => {
    ...
    const generated = generateSessionDates(day, editableDates.length || group.sessionCount || 4);
  ...
  const handleAddSession = () => {
    ...
    if (lastDate) {
      const parsed = new Date(lastDate.replace(/\//g, '-'));
      if (!isNaN(parsed.getTime())) {
        parsed.setDate(parsed.getDate() + 7);
        nextDateStr = formatToYYYYMMDD(parsed);
      }
    }
    if (!nextDateStr) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      >
        <span style={{ fontWeight: 700 }}>دليل الحضور (اضغط على الخانة للتبديل):</span>
                ...
                return (
                  <tr key={student.rowId}>
                    ...
                    {Array.from({ length: group.sessionDates.length }).map((_, sessionIdx) => {
                      ...
                      return (
                        <td
                          ...
                          title="اضغط لتغيير حالة الحضور (P / A / M)"
                        >
                          <div
                            style={{
                              width: '34px',
                    ...
                    <td style={{ textAlign: 'center', fontWeight: 800 }}>
                      {student.totalAttendance} / {group.sessionDates.length}
                    </td>
